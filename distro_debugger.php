...
function renderLogSection(array $source): void
{
    ...
    if ($lorkhanLog) {
        // Keep Lorkhan's bounded/redacted reader; missing helpers must never expose raw logs.
        $root = dirname(__DIR__) . '/LorkhanServer';
        if (!is_file($root . '/lib/Autoload.php')) $root = '/var/www/html/LorkhanServer';
        if (is_file($root . '/lib/Autoload.php') && is_file($root . '/ui/tmpl/server_log_reader.php')) {
            require_once $root . '/lib/Autoload.php';
            require_once $root . '/ui/tmpl/server_log_reader.php';
            $tail = lorkhan_ui_log_tail($resolvedPath);
            $readable = $tail !== null;
            $rawLines = $readable ? (preg_split('/\R/u', lorkhan_ui_redact_log($tail)) ?: []) : [];
        } else {
            $readable = false;
            $apiError = 'Install LorkhanServer to view its redacted service logs.';
        }
    }
...
<main class="debugger-shell">
    ...
    <div class="tab-nav" role="tablist" aria-label="Debugger Tabs">
        ...
        <button class="tab-button<?= $initialServerTab === 'lorkhan' ? ' active' : '' ?>" type="button" data-tab="lorkhan" role="tab" aria-selected="<?= $initialServerTab === 'lorkhan' ? 'true' : 'false' ?>" aria-controls="tab-lorkhan">LORKHAN</button>
    </div>
