...
function sm_guard(string $kind, string $mod): array
{
    ...
    if ($method === 'GET' && in_array($queryAction, $legacyActions, true)) {
        http_response_code(405);
        throw new StorageManagerRequestException('This action now requires confirmation. Open Playthrough Saves and use its action button. Nothing was changed.');
    }
    if ($method === 'POST') {
        ...
        if (!is_string($token) || !hash_equals($_SESSION['storage_csrf'], $token)
            ...
            throw new StorageManagerRequestException('Security check failed. Reload Playthrough Saves and try again. Nothing was changed.');
        }
